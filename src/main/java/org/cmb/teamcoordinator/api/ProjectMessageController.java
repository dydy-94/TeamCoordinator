...
public class ProjectMessageController {
    ...
    public ResponseEntity<MessageAcceptedResponse> submit(
        ...
        MessageAcceptedResponse response =
                messageService.accept(identityProvider.currentIdentity(servletRequest), projectId, request);
    ...
    public SseEmitter events(
        ...
        messageService.requireEventAccess(identity, projectId);
        return streamHub.subscribe(
                identity.getTenantId(),
                projectId,
                afterSequence,
                () -> messageService.replayAuthorized(identity, projectId, afterSequence));
    }
