CREATE TABLE project_artifact (
    ...
    CONSTRAINT fk_artifact_task FOREIGN KEY (task_id) REFERENCES coordinator_task (id)
);

CREATE INDEX idx_artifact_project_status
    ON project_artifact (tenant_id, project_id, status, created_at);

CREATE TABLE project_artifact_lineage (
