...
conversation is created. It remains unchanged for the entire conversation and
is sent to AgentCore on every submit, stream, status, cancel and resume call.
...
```json
{
  ...
  "idempotency_key": "project-001-message-001"
}
```
...
| `idempotency_key` | Yes | Maximum 128 characters |
...
Submitting the same idempotency key returns the existing accepted message.
...
| Cancel | `POST /runs/{sessionId}/cancel` | `AGENTCORE_CANCEL_PATH` |
| Resume | `POST /runs/{sessionId}/resume` | Application property |
...
Every operation also includes the Task's stable business session:
...
{
  "expertId": "analysis-expert",
  "taskText": "Analyze API risks",
  "structuredInput": {},
  "attachmentRefs": ["minio-storage-key"],
  "idempotencyKey": "task-request-id"
}
```
...
```json
{
  "expertId": "coordinator",
  "taskText": "分析接口风险并撰写报告",
  "structuredInput": {
    "operation": "ANALYZE",
    "prompt": "...",
    "context": {
      "projectName": "Risk Review",
      "projectDescription": "Release review",
      "text": "分析接口风险并撰写报告",
      "recentMessages": [],
      "experts": [],
      "attachments": [],
  ...
  "attachmentRefs": [],
  "idempotencyKey": "coordinator-run-...:ANALYZE"
}
```
...
id: 1
event: RUN_ACCEPTED
data: {"sessionId":"run-...","eventId":"run-...:1","sequence":1,"type":"RUN_ACCEPTED","status":"ACCEPTED","message":"Run accepted.","createdAt":"2026-08-03T10:00:00Z","payload":{}}

id: 2
event: RUN_PROGRESS
data: {"sessionId":"run-...","eventId":"run-...:2","sequence":2,"type":"RUN_PROGRESS","status":"RUNNING","message":"Working.","createdAt":"2026-08-03T10:00:01Z","payload":{}}
...
|---|---:|---|
| `sessionId` | Yes | Run session |
| `eventId` | Recommended | Globally stable event identifier |
| `sequence` | Yes | Monotonically increasing cursor within a run |
| `type` | Yes | Event type |
| `status` | Yes | Current run state |
| `message` | No | Human-readable progress |
| `createdAt` | Recommended | Event timestamp |
...
Coordinator deduplicates expert events by event ID and sequence and persists
