...
- **Identity contract**: `X-Tenant-Id` + `X-User-Id` headers (`HeaderIdentityProvider`). Tenant gate is fail-closed: the tenant must exist and be ACTIVE (`digital_team_tenant`) and the user must be a member (`digital_team_tenant_user`; users come from an external login system — no user table). Errors: 404 `TENANT_NOT_FOUND`, 403 `TENANT_DISABLED`, 403 `TENANT_ACCESS_FORBIDDEN`. Platform admins (`PLATFORM_ADMIN_USERS`, default `root`, comma-separated ∪ `digital_team_platform_admin` table — direct INSERT grants admin, no API) bypass the membership check (any ACTIVE tenant) and manage tenants via `/api/v1/admin/tenants`; `GET /api/v1/tenants` (requires only `X-User-Id`) feeds the frontend tenant switcher — it returns all tenants for platform admins. Project authorization is role-based (OWNER, MEMBER, VIEWER).
